...
SIZE_OF_STREAM = BATCH * 1100
SIZE_OF_TASK = BATCH * 100
...
PRETRAIN_NUM_PER_CLASS = 500
BUFFER_NUM_PER_CLASS   = 50
#AVOID OOM
TEST_BATCH = 100000 // (4*len(TASK_CLASSES))
...
def show_samples(dts, sample_per_row=3):
    ...
    for i, (img, lbl) in enumerate(rand_dts):
        ...
        plt.imshow(show_img)
        plt.title(str(lbl).replace("tf.Tensor","")[:32])
    plt.show()
...
def stream_testing(model, dts):
    ...
    for count, (x,y) in enumerate(dts,1):
...
#    )
    if MODEL_NAME == "resnet32":
        init_model = ResNet32(pretrain_img_shape, len(pretrain_n_classes))
    if MODEL_NAME == "resnet18":
        init_model = ResNet18(pretrain_img_shape, len(pretrain_n_classes))
...
with tf.device("/GPU:0"):
    ...
    stream_ = stream_\
        .apply(tf.data.experimental.prefetch_to_device("/GPU:0", buffer_size=5))
    ##################################################
    ...
    for cur_batch, (images, labels) in enumerate(stream_):
        ...
        for method in METHODS:
            TT = time()
            if cur_batch==0 or (cur_batch+1)%TEST_FREQ==0:
                accuracy_per_task = stream_testing(method.model, test_stream)
                last_accuracy_per_method[method.name] = accuracy_per_task[:]
            else:
                accuracy_per_task = last_accuracy_per_method.get(method.name)
            ET = time()
            full_run += (ET-TT)/3600
            # Training code here
